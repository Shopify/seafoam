...
class GraalVM < Struct.new(:url, :dir)
  def name
    dir
  end
...
Dir.chdir 'tools' do
  GraalVMs.each do |graalvm|
    ...
    Dir.chdir graalvm.name do
      ...
      system 'rm', '-rf', 'graal_dumps'
      pipe = IO.popen(["#{bin}/java", '-XX:-UseOnStackReplacement', '-XX:CompileCommand=dontinline,*::*', '-XX:+PrintCompilation', '-Dgraal.PrintBackendCFG=true', '-Dgraal.PrintGraphWithSchedule=true', '-Dgraal.Dump=:3', 'JavaExamples'])
      loop do
      ...
      Dir.glob 'graal_dumps/**/*\\[JavaExamples\\.example*.bgv' do |bgv|
        cfg = Pathname.new(bgv).sub_ext('.cfg').to_s
        bgv =~ /HotSpotCompilation-\d+\[JavaExamples\.(.+)\(/
        method = $1
        system 'cp', bgv, "#{method}.bgv"
        system 'cp', cfg, "#{method}.cfg"
      ...
      system 'rm', '-rf', 'graal_dumps'
      pipe = IO.popen(["#{bin}/node", '--experimental-options', '--engine.CompileOnly=fib', '--engine.TraceCompilation', '--engine.MultiTier=false', '--engine.Inlining=false', '--vm.Dgraal.Dump=Truffle:1', '--vm.Dgraal.PrintBackendCFG=true', '../../examples/fib.js', '14'], {:err=>[:child, :out]})
      loop do
        line = pipe.gets
      ...
      system 'rm', '-rf', 'graal_dumps'
      pipe = IO.popen(["#{bin}/ruby", '--jvm', '--experimental-options', '--engine.CompileOnly=fib', '--engine.TraceCompilation', '--engine.NodeSourcePositions', '--engine.MultiTier=false', '--engine.Inlining=false', '--vm.Dgraal.Dump=Truffle:1', '--vm.Dgraal.PrintBackendCFG=true', '../../examples/fib.rb', '14'], {:err=>[:child, :out]})
      loop do
        line = pipe.gets
