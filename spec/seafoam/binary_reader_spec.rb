...
describe Seafoam::BinaryReader do
  describe '#read_utf8' do
    it 'reads a UTF-8 string' do
      string = 'abc😀'
      reader = Seafoam::BinaryReader.new(StringIO.new(string))
  ...
  describe '#read_float64' do
    it 'reads a double-precision float' do
      reader = Seafoam::BinaryReader.new(StringIO.new("#{[1.5].pack('G')}hello"))
  ...
  describe '#read_sint64' do
    it 'reads a signed 64-bit network-endian integer' do
      reader = Seafoam::BinaryReader.new(StringIO.new("#{[1234].pack('q>')}hello"))
  ...
  describe '#read_sint32' do
    it 'reads a signed 32-bit network-endian integer' do
      reader = Seafoam::BinaryReader.new(StringIO.new("#{[1234].pack('l>')}hello"))
  ...
  describe '#read_sint16' do
    it 'reads a signed 16-bit network-endian integer' do
      reader = Seafoam::BinaryReader.new(StringIO.new("#{[1234].pack('s>')}hello"))
  ...
  describe '#read_sint8' do
    it 'reads a signed 8-bit integer' do
      reader = Seafoam::BinaryReader.new(StringIO.new("#{[-123].pack('c')}hello"))
  ...
  describe '#read_uint8' do
    it 'reads an unsigned 8-bit integer' do
      reader = Seafoam::BinaryReader.new(StringIO.new("#{[212].pack('C')}hello"))
  ...
  describe '#skip_float64' do
    it 'skips 8 bytes' do
      reader = Seafoam::BinaryReader.new(StringIO.new('12345678x'))
      reader.skip_float64
      expect(reader.read_uint8).to eq 'x'.ord
    end

    it 'accepts a count' do
      reader = Seafoam::BinaryReader.new(StringIO.new('1234567812345678x'))
  ...
  describe '#skip_float32' do
    it 'skips 4 bytes' do
      reader = Seafoam::BinaryReader.new(StringIO.new('1234x'))
      reader.skip_float32
      expect(reader.read_uint8).to eq 'x'.ord
    end

    it 'accepts a count' do
      reader = Seafoam::BinaryReader.new(StringIO.new('12341234x'))
    ...
    describe '#skip_int16' do
      it 'skips 2 bytes' do
        reader = Seafoam::BinaryReader.new(StringIO.new('12x'))
        reader.skip_int16
        expect(reader.read_uint8).to eq 'x'.ord
      end

      it 'accepts a count' do
        reader = Seafoam::BinaryReader.new(StringIO.new('1212x'))
        reader.skip_int16 2
        expect(reader.read_uint8).to eq 'x'.ord
      end
    end
    ...
    describe '#skip_int8' do
      it 'skips 1 byte' do
        reader = Seafoam::BinaryReader.new(StringIO.new('1x'))
        reader.skip_int8
        expect(reader.read_uint8).to eq 'x'.ord
      end

      it 'accepts a count' do
        reader = Seafoam::BinaryReader.new(StringIO.new('11x'))
        reader.skip_int8 2
        expect(reader.read_uint8).to eq 'x'.ord
      end
    end
    ...
    describe '#skip' do
      it 'skips n bytes' do
        reader = Seafoam::BinaryReader.new(StringIO.new('123x'))
        reader.skip 3
        expect(reader.read_uint8).to eq 'x'.ord
      end
    end
    ...
    describe '#eof?' do
      it 'returns false when not at the end of file' do
        reader = Seafoam::BinaryReader.new(StringIO.new('123'))
        expect(reader.eof?).to be false
      end

      it 'returns true when at the end of file' do
        reader = Seafoam::BinaryReader.new(StringIO.new(''))
